...
Try {
	...
	[string]$appScriptDate = '07/15/2022'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		If(-Not (Test-Path -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{B9E46F95-AC34-4943-AFE2-B72EFD56C6C0}")){
			...
			$exitCode = Execute-Process -Path "$dirFiles\windowsdesktop-runtime-6.0.6-win-x64.exe" -Parameters '/install /quiet /norestart' -PassThru
			If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		}
		Else{
