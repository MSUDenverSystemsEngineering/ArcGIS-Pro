...
Try {
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		If(-Not (Test-Path -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\{B9E46F95-AC34-4943-AFE2-B72EFD56C6C0}")){
			Write-Log -Message "Attempting to run Desktop Runtime 6.0.6 installer..." -Source 'Installation' -LogType 'CMTrace'
			$exitCode = Execute-Process -Path "$dirFiles\windowsdesktop-runtime-6.0.6-win-x64.exe" -Parameters '/install /quiet /norestart' -IgnoreExitCodes '1638' -PassThru
			If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		}
		Else{
			Write-Log -Message "Desktop Runtime 6.0.6 is already installed" -Source 'Installation' -LogType 'CMTrace'
		}
		$exitCode = Execute-MSI -Action Install -Path "$dirFiles\ArcGISPro.msi" -Parameters 'ALLUSERS=1 CHECKFORUPDATESATSTARTUP=0 SOFTWARE_CLASS=Professional AUTHORIZATION_TYPE=NAMED_USER License_URL="https://msudenver.maps.arcgis.com" ENABLEEUEI=0 ACCEPTEULA=YES /qn' -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
		...
		Execute-MSP -Path "$dirFiles\ArcGIS_Pro_311_185017.msp" '/qn'
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		$exitCode = Execute-Process -Path "msiexec.exe" -Parameters "/x{A61AD307-865F-429F-B2A3-5618BD333F7E} /quiet /norestart /log $Env:windir\Logs\Software\ArcGISPro3.1-uninstall.log" -PassThru
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDhBxNv07S5EnqR
# zC7HleZiqhK2cRlqHMbkjiMVrKswoqCCH8EwggVvMIIEV6ADAgECAhBI/JO0YFWU
...
# AYI3AgELMQ4wDAYKKwYBBAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQgRi8hnpDu+nQe
# 1i+xtRZITVhwN5XdPKuzHQT1QdbZRSYwDQYJKoZIhvcNAQEBBQAEggGAUA6BI5dJ
# 4p0f3Gm8zqvqPYQqhRw+ZGo4DdKtNEAZyqhGvG9JGV5H01XT89cOWCVWUHJbaiMc
# tWwExalresh8IuWbSMU50YuT3UjC1lHCbMBEuFjnjl6eg9TXk/+3GJHFSlaYYlcu
# mrFFejJy+J8CHypqcJDyS6RWPqJmur6cvICC6ucuxU+xYYfPzGNTLgUnaBu9VLgT
# vO5Lf0u71md9M17lA1e81otSE57AAmIgtv+SteTaJ8I0g/3/qUo4rCNOgdiLqw/X
# M7ZI4oLGY7XymZ4QnVtCZghjTS/C7y/DnZ2EpZLhkBTj1OxHvRoNiwbah5ALCvuy
# OhLG7MeTDqiDJeb8Otvv82wm66HWseOYQCvGfHMpy9+eevHXRrFyp6CVMhh0BDRF
# Oe5KEeei5JRs35+BIXilp4vhC3n6gxJUbw8Y4X3AGks9MMvFt/jw89NAkciB6iug
# xEUHQpYHduMgXZqG/1Dr8IXB5Fs0ibHxK0rHAhSn0oyW64cgrWYbTQQhoYIDTDCC
...
# BgkqhkiG9w0BCQMxCwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzA0MTkx
# NjAxMjJaMD8GCSqGSIb3DQEJBDEyBDBbIHQ88/VBMh1v8qTdqTbQh71Zefi+rmvh
# 5xlfEimqpnC1UZ5IIQ1DskoVIPSoJaAwDQYJKoZIhvcNAQEBBQAEggIAQtgkbPF8
# eO8CeqWf5pR1PsSLeCxKEsoP1glGjrDub97sLjAonAu9c9AGgYpwSHsTE2TunU1b
# A5DSJIbonDv9wx+Iu5rPd1hAPjBBjfyRFicUkKYxxSh2kTcEl9K3YXHr0dccmUmf
# MhiimSlc3sqyXqDyVo3XyMJ6XSe34FRMtap4yJZK/cyK17czH5E2hARlSaCcR5yy
# evAm05sODPWX8wekkMqq5+xqHGYr6T6iXj86l/dAMJ9lA2Z8DoEDCixc8/AgHPOK
# xa3Dkcyp7kfRZEyKVf4IBS7gRB+VPB5Pfowp9nismGCXamb1Qp+6dtGEZi300j0c
# qbuox7mzDTfbQjS+xgSHVk6TRdjCVR9wH9XLrrc/hRvYjx5we0OWxqze+ew5KtpF
# +GpoNJZvKBUjuIdjkzBQrop7EF7Z9HfWhWDLPfyeYB2lS0Hi51jzbqf2GL4Vho3e
# SDx/iDA21e2af5CucNJ2d1ddPDIumv+aJ9GJpZZAwOfLvSB5D7VaYDcZtL90U2t9
# 0jYoFL0FQFc3rciWMkHLsv1I3bmDuZDNqQBjjxDCfs/CDOL5tW4DwzSzsemzNe9s
# rCc4oLh8ZjWJZsVf/8mqKZL+EtDNoLJmlt5tby6Yoi9OUkBHTPfdBmdaDoVGqETx
# CPHsfvVUADxr299lQ7maQXUUxya9gNB8Gu8=
